...
<div class="col-xs-12 col-md-12 text-center">
	 <h2><label><span class="glyphicon glyphicon-barcode"></span> Editar Orçamento</label></h2>
 </div>
<form action="${linkTo[OrcamentoController].editado(null) }" method="post" role="form">
...
<input type="hidden" id="orcamento_id" value="${orcamento.id }" name="orcamento.id" />
<input type="hidden" id="orcamento_atendido" value="${orcamento.atendido }" name="orcamento.atendido" />
<input type="hidden" id="id_solicitacao" value="${orcamento.solicitacao.id}" name="id_solicitacao" />
<input type="text" class="form-control" readonly="readonly" id="solicitacao_selecionada" value="${orcamento.solicitacao.solicitacoes }" />
</div>
...
<div class="col-sm-6 col-md-4 col-lg-2">
  ...
  <div class="col-sm-6 col-md-4 col-lg-2">
    ...
    <input type="text" class="form-control dvalor" id="txt_valor_desconto"  value="${orcamento.desconto }"/>
   	</div>
   	</div>
   	<div class="col-sm-6 col-md-4 col-lg-2">
   		<label id="lbl_total_desconto">Total De Desconto R$:</label>
   		<input type="text" readonly="readonly" class="form-control"
   		 id="txt_total_de_desconto" name="orcamento.desconto" value="${orcamento.desconto }" />
   </div>
<div class="col-sm-12 col-md-8 col-lg-3">
<label>Total:</label>
<div class="input-group">
  <span class="input-group-addon" id="sizing-addon1">R$</span>
  <input type="text" class="form-control" readonly="readonly" name="orcamento.total" value="${orcamento.total }" id="total_gasto"/>
</div>
</div>
<div class="col-sm-12">
...
<script type="text/javascript">
$("#corpoComprado").append("<tr  class='produto_gasto' id='tr_peca_"+num_pecas+"'><td>${peca.peca.nome}</td><td>${peca.preco_venda}"+
		"</td><td>${peca.quantidade_usada}</td>"+
...
$("#resto").append("<div id='div_pecas_id_"+num_pecas
		...
		+"].quantidade_usada' value='${peca.quantidade_usada}' />"+
		"<input type='hidden' name='produtos["+num_pecas+"].preco_compra'"+
		" value='${peca.preco_compra}' /><input type='hidden' name='produtos["+num_pecas
		+"].preco_venda' value='${peca.preco_venda}' />"+
		"<input type='hidden' name='produtos["+num_pecas
		+"].peca.id' value='${peca.peca.id}' /></div>");
num_pecas++;
</script>
</c:forEach>
<script type="text/javascript">
...
<script type="text/javascript">
$("#corpoComprado").append("<tr class='servico_gasto' id='tr_servico_"+num_servico+"'><td>${servico.servico.nome}</td><td>${servico.preco}</td><td>-</td>"+
		"<td><a href='#' onclick='remover_servico_tabela("+num_servico+");'>"+
		"<span class='glyphicon glyphicon-trash'></span></a></td></tr>");	
$("#resto").append("<div id='div_servicos_id_"+num_servico+"'><input type='hidden' name='servicos["+num_servico+"].preco'"+
" value='${servico.preco}' /><input type='hidden' name='servicos["+num_servico+"].numero_horas'"+
" value='${servico.numero_horas}' /><input type='hidden' name='servicos["+num_servico+"].servico.id' value='${servico.servico.id}'/></div>");
num_servico++;
</script>
</c:forEach>
<script type="text/javascript">
...
var primeira_autorizacao = true;
function autorizar(){

	var orc_desc = "${orcamento.desconto}";
		var orc_tot_desc = $("#txt_total_de_desconto").val();
		if (Number(orc_desc) != Number(orc_tot_desc)) {
			if (!ja_autorizou) {
				ja_autorizou = true;
...
<script>
	...
	var desconto = 0;
	var mudar_desconto_auth = function(){
		autorizar();
		mudar_desconto();
	};
	...
	var editar_desc = "${orcamento.desconto}";
	if (Number(editar_desc) > 0) {
		
	}
	$("#cbx_desconto").change(mudar_desconto_auth());
	$("#cbx_desconto").on("focus", mudar_desconto_auth());
	$("#cbx_desconto").on("keydown", mudar_desconto_auth());
	$("#cbx_desconto").on("keyup", mudar_desconto_auth());
