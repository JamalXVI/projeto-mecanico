	...
	<div class="col-xs-12 col-md-12 text-center">
	 <h2><label><span class="glyphicon glyphicon-user"></span> Visualizar Cliente</label></h2>
 </div>
    <div class="col-md-4 col-xs-12 col-lg-2">
    <img src="<c:url value="/Imagem/Cliente/${cliente.id }/" />" width="180" height="180"
    class="img-circle"/>
    </div>
    ...
    <label for="nome">Nome:</label>
        ...
        value="${cliente.nome}" readonly="readonly"/>
     	...
     	<c:if test="${cliente.identificacao.tipo == 'CPF' }">
     	 	...
     	 	<label>CPF</label>
     	 	<input type="text" value="${cliente.identificacao.cpf }" class="form-control" readonly="readonly">
     	 	</div>
     	</c:if>
     	<c:if test="${cliente.identificacao.tipo != 'CPF' }">
     	 	<div class="col-md-4 col-xs-8 col-lg-2">
     	 	<label>CNPJ</label>
     	 	<input type="text" value="${cliente.identificacao.cnpj }" class="form-control" readonly="readonly">
     	 	</div>
     	 	<div class="col-md-4 col-xs-8 col-lg-2">
     	 	<label>IE</label>
     	 	<input type="text" value="${cliente.identificacao.inscricao_estadual }" class="form-control" readonly="readonly">
     	 	</div>
     	</c:if>
     <div  class="col-md-4 col-xs-8 col-lg-2">
        <label for="rua">Endereço:</label>
        <input type="text" name="cliente.rua" id="rua" class="form-control"
        value="${cliente.rua}" readonly="readonly"/>
      </div>
        <div  class="col-md-4 col-xs-4 col-lg-1">
        <label for="num">Número:</label>
        <input type="text" name="cliente.num" id="num" class="form-control"
        value="${cliente.num}" readonly="readonly"/>
        </div>
        <div  class="col-md-4 col-xs-12 col-lg-2">
        <label for="comp">Complemento:</label>
        <input type="text" name="cliente.comp" id="comp" class="form-control"
        value="${cliente.comp}" readonly="readonly"/>
        </div>
        <div  class="col-md-4 col-xs-12 col-lg-2">
        <label for="bai">Bairro:</label>
        <input type="text" name="cliente.bai" id="bai" class="form-control"
        value="${cliente.bai}" readonly="readonly"/>
        </div>
        <div  class="col-md-4 col-xs-12  col-lg-2">
        ...
        value="${cliente.email}" readonly="readonly"/>
        </div>
        ...
        <h3 style="display: inline;">Veículos</h3>
        	<a 
        	href="${linkTo[VeiculoController].cadastrar_c(cliente.id)}" class="mais">
...
<i class="glyphicon glyphicon-plus glyphicon-stack glyphicon-stack-1x"></i>
        ...
        <tbody> 
        <c:forEach items="${cliente.veiculos }" var="veiculo">
        ...
        <a href="${linkTo[PenduraController].lista_cliente(cliente.id) }"><span class="glyphicon glyphicon-list-alt"></span></a>
        </div>
